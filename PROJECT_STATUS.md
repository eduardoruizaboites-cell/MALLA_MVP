...
── ENTRADA — 2026-09-25 21:30 (Iteración 47: aceptación bidireccional por GATT) ──
Compilación: BUILD SUCCESSFUL (in 1m 22s, :app).
Commit: 14f4d925 + fc126005
QUÉ SE HIZO: Cerrar el flujo de agregar contacto end-to-end. Antes: A mandaba invitación → B aceptaba → B tenía la conversación pero A nunca se enteraba. Ahora: el ACCEPT viaja de vuelta al emisor y ambos crean el contacto + conversación. Fix arquitectónicamente limpio usando MallaEventBus como puente (mismo patrón que iter 46). 4 cambios: (1) MallaEventBus.kt: nuevo canal acceptanceReceived: SharedFlow<Triple<String,String,Int>>. (2) BleTransport.kt: nuevo SharedFlow acceptancePayloads + filtro 'ACCEPT|' en el handler de INVITE_CHAR_UUID. Antes sendAcceptance usaba serviceData del advertising, pero el scanner solo lee manufacturerData (0xABCD), así que nunca llegaba. (3) InvitationManager.sendAcceptance reescrito: en lugar de startAdvertisingWithPayload, hace BleManager.connectAndWriteData(device, INVITE_CHAR_UUID, payload) — mismo canal GATT que la invitación. (4) MainActivity.kt: nuevo LaunchedEffect que observa MallaEventBus.acceptanceReceived, crea ContactEntity + ConversationEntity del lado emisor, y loguea 'ACEPTACION recibida de X'.
¿ERA UN FIX DE ERROR?: SÍ, 2 bugs coordinados. ERROR A1: el scanner de BleManager solo lee manufacturerData (0xABCD), pero sendAcceptance usaba serviceData con ParcelUuid. Los formatos no se hablaban. SOLUCIÓN: ACCEPT por el mismo canal GATT que la invitación. ERROR A2: acceptanceCallback nunca se registraba en MainActivity. SOLUCIÓN: nuevo observer via MallaEventBus que reemplaza el callback huérfano.
HIPÓTESIS DESCARTADAS (ROL 2): (1) 'el emisor no procesa el ACCEPT' — DESCARTADA: no lo recibía, no lo procesaba. (2) 'el advertising del ACCEPT no se emite' — DESCARTADA: se emitía, pero con serviceData, y el scanner no lo veía. (3) 'falta callback en MainActivity' — CONFIRMADA parcialmente; el grep mostró que acceptanceCallback nunca se setea.
VERIFICADO EN: dispositivo real (log 2026-09-25 19:00 y 20:18): '[MAIN] ACEPTACION recibida de Lalo' + '[MAIN] Contacto Lalo guardado tras recibir ACCEPT' en el emisor.
COMPATIBILIDAD CONSIDERADA (R21): Android 11 + Android 16. Sin cambios de wire format (el payload ACCEPT|user|name|seed usa el mismo canal INVITE_CHAR_UUID y fragmentación).
SUGERENCIAS PROACTIVAS OFRECIDAS (R20, sin implementar aún): (1) si el emisor no está conectado al receptor cuando llega el ACCEPT, se pierde — no hay store-and-forward todavía. (2) formato del payload 'ACCEPT|' podría colisionar con nombres de usuario que empiecen con ese literal; conviene un prefijo binario en el futuro.
DEUDA / PENDIENTE QUE SIGUE ABIERTA: store-and-forward de ACCEPT si el emisor no está; iter 48-50.
──────────────────────────────


── ENTRADA — 2026-09-25 21:32 (Iteración 48: notificación premium con tema) ──
Compilación: BUILD SUCCESSFUL (in 1m 22s, :app).
Commit: 14f4d925 + fc126005
QUÉ SE HIZO: Reescritura de showMessageNotification en NotificationHelper.kt para que sea visible en pantalla bloqueada y adopte el color del tema del usuario. Cambios: (1) MessagingStyle (mismo look de WhatsApp/Telegram para agrupar mensajes). (2) setCategory(CATEGORY_MESSAGE) + setVisibility(VISIBILITY_PRIVATE) para prioridad en lock screen. (3) setColor(theme.primary) — el color de acento viene del MallaColorScheme activo del usuario (cyan, verde, azul, etc.). (4) setDefaults(DEFAULT_VIBRATE) + setShowWhen(true). (5) Ícono ic_notification.xml cambiado a tint blanco (#FFFFFF) — antes tenía tint cyan #00E5FF que Android no renderiza bien en la barra de estado. Conversión manual de Compose Color a Int via Color.red/green/blue (evita depender de androidx.compose.ui.graphics.toArgb que varió entre versiones).
¿ERA UN FIX DE ERROR?: NO es fix de error funcional — mejora UX. El mensaje ya llegaba con IMPORTANCE_HIGH, pero no aparecía en lock screen por falta de CATEGORY_MESSAGE + VISIBILITY_PRIVATE. Y el color era fijo (no respetaba el tema).
HIPÓTESIS DESCARTADAS (ROL 2): (1) 'el permiso POST_NOTIFICATIONS está denegado' — DESCARTADA: el grep mostró que se declara en manifest y se pide en runtime. (2) 'el canal está bloqueado' — DESCARTADA: el log 'NotifHelper: Canal malla_messages creado. Existe=true, importance=4, enabled=true' confirma que está activo. (3) 'toArgb() no está disponible' — CONFIRMADA: error de compilación 'Unresolved reference: toArgb'. SOLUCIÓN: conversión manual con Color.red/green/blue.
VERIFICADO EN: compilación limpia. Pendiente verificación en device.
COMPATIBILIDAD CONSIDERADA (R21): Android 8+ (NotificationChannel). MessagingStyle desde API 24. Color.red/green/blue disponibles en todas las versiones modernas de Compose.
SUGERENCIAS PROACTIVAS OFRECIDAS (R20, sin implementar aún): (1) agregar respuesta rápida directa desde la notificación (RemoteInput). (2) ícono propio monocromático vectorial en vez del path genérico actual. (3) si el usuario tiene 'contenido sensible oculto', usar setPublicVersion() para mostrar 'Nuevo mensaje' en lock screen.
DEUDA / PENDIENTE QUE SIGUE ABIERTA: test visual de notificación en device con distintos temas; iter 49-50.
──────────────────────────────


── ENTRADA — 2026-09-25 21:34 (Iteración 49: fix crash Toast en hilo secundario) ──
Compilación: BUILD SUCCESSFUL (in 1m 22s, :app).
Commit: fc126005
QUÉ SE HIZO: Fix del crash 'Can't toast on a thread that has not called Looper.prepare()' que se disparaba al abrir la conversación después de aceptar una invitación, cerrando ambas apps simultáneamente. Causa raíz: Toast.makeText() llamado desde corrutinas en Dispatchers.IO o desde withContext(Dispatchers.Main) dentro de una coroutine ya cancelada — el switch al main looper no completa y el Toast corre en un thread sin Looper. Fix: (1) nuevo ToastHelper.kt global en :app/util con Handler(Looper.getMainLooper()).post { Toast.makeText(...).show() } — garantiza ejecución en main looper sin importar el estado de la coroutine. (2) reemplazo global de Toast.makeText(...).show() por ToastHelper.show(...) en 11 archivos (ChatInputBar, VoiceRecorderButton, CodeInputScreen, ContactsScreen, ConversationsScreen, FaroScreen, PerfilScreen, RegistrationScreen, IncomingRequestDialog, CascadeRouter, MainActivity). (3) nuevo CrashCapture.kt — handler global de UncaughtException que escribe el stack trace completo al malla_diagnostics.txt antes de matar el proceso. Permite diagnosticar crashes sin cable USB ni adb logcat. (4) MainActivity llama CrashCapture.install(this) en onCreate.
¿ERA UN FIX DE ERROR?: SÍ, bloqueante. ERROR: 'Can't toast on a thread that has not called Looper.prepare()' cerraba ambas apps al abrir el chat. SOLUCIÓN: ToastHelper global + CrashCapture. ¿FUNCIONÓ?: SÍ, verificado en device (log 2026-09-25 20:18 en adelante: no más crashes, mensajes, fotos, emojis y zumbidos enviados sin interrupciones).
HIPÓTESIS DESCARTADAS (ROL 2): (1) 'el fix toastSafe en MainActivity resuelve todo' — DESCARTADA: el crash persistía porque había Toasts en otros archivos (ChatInputBar, VoiceRecorderButton, etc.) que también se disparaban al abrir el chat. (2) 'es un problema de Context' — DESCARTADA: el problema era el thread, no el context. (3) 'algún callback del sistema' — DESCARTADA: era Toast.makeText desde coroutines.
VERIFICADO EN: dispositivo real. Log 2026-09-25 20:15 en adelante: sin crashes, con todas las funciones operativas.
COMPATIBILIDAD CONSIDERADA (R21): Handler(Looper.getMainLooper()) disponible desde API 1. Sin cambios de protocolo.
SUGERENCIAS PROACTIVAS OFRECIDAS (R20, sin implementar aún): (1) reemplazar los últimos Toasts que puedan quedar en componentes muy anidados. (2) considerar una Snackbar global en vez de Toast para feedback in-app más premium. (3) CrashCapture podría subir el stack a un endpoint opcional (con consentimiento) para monitoreo agregado.
DEUDA / PENDIENTE QUE SIGUE ABIERTA: iter 50; revisión final de Toasts que puedan haber quedado.
──────────────────────────────


── ENTRADA — 2026-09-25 21:36 (Iteración 50: card in-app + notificación al recibir ACCEPT) ──
Compilación: BUILD SUCCESSFUL (in 1m 22s, :app).
Commit: fc126005
QUÉ SE HIZO: Feedback premium cuando otro usuario nos acepta la invitación. Antes: solo un Toast gris 'X te agregó'. Ahora: bifurcación foreground/background. (1) Nuevo composable AcceptanceReceivedCard.kt: card flotante pegado al bottom, avatar 56dp con glow cyan pulsante detrás (mismo lenguaje visual de IncomingRequestDialog), texto 'X te agregó' + 'Presiona para abrir el chat', botón cyan 'Abrir' + texto 'Cerrar', slide-up spring 300ms, háptic al aparecer, auto-dismiss 8s. (2) Nuevo método NotificationHelper.showAcceptanceNotification(context, acceptorUserId, acceptorName): notificación del sistema con category=SOCIAL, priority=HIGH, visibility=PRIVATE, BigTextStyle, color de acento del tema, vibración, tap → abre MALLA en la conversación con ese usuario. (3) MainActivity: state var showAcceptanceCard: Triple<String,String,Int>?, y en el observer de acceptanceReceived se bifurca: si appForeground → set card state, sino → showAcceptanceNotification. (4) Render del card justo antes de effectiveScheme, con onOpenChat navegando a la conversación.
¿ERA UN FIX DE ERROR?: NO es fix — mejora UX premium. El usuario pedía 'una pantalla o notificación de que el otro usuario te aceptó, no solo un toast'.
HIPÓTESIS DESCARTADAS (ROL 2): (1) 'reutilizar IncomingRequestDialog para el card de aceptación' — DESCARTADA: son momentos UX distintos (receptor vs emisor) y el diseño requiere textos/acciones diferentes. Se creó componente nuevo reutilizando el estilo visual. (2) 'NotificationHelper ya maneja acceptance' — DESCARTADA: solo manejaba mensajes y discovery. Se agregó método específico.
VERIFICADO EN: compilación limpia. Pendiente verificación en device.
COMPATIBILIDAD CONSIDERADA (R21): Android 8+ (NotificationChannel API). AcceptanceReceivedCard usa Compose con las mismas APIs que IncomingRequestDialog (ya verificado en gama baja). Auto-dismiss con LaunchedEffect(Unit) — se cancela limpiamente si el usuario desmonta la composición.
SUGERENCIAS PROACTIVAS OFRECIDAS (R20, sin implementar aún): (1) revisar si 8s de auto-dismiss es el tiempo ideal o conviene hacerlo configurable (5s/10s). (2) agregar swipe-to-dismiss al card. (3) animación de entrada más elaborada (fade + scale) si el usuario quiere más 'premium'.
DEUDA / PENDIENTE QUE SIGUE ABIERTA: test en device del card in-app + notificación system; verificar cambio de color al cambiar tema; ajustar duración del auto-dismiss según feedback.
──────────────────────────────
